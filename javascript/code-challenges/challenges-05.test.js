...
const toLastNames = people => {
  // Solution code here...
  let arr = people.map(function(element) {
...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((accumlator, value) => {
    accumlator = accumlator + value.purchasePrice;
    return accumlator;
  }, 0);
};
...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((accumlator, character, idx) => {
    accumlator[idx] = character.name;
    return accumlator;
  }, []);
};
...
const extractStats = (snorlaxData, arr) => {
  // Solution code here...
  return arr.reduce((accumulator, value) => {
    if (value.stat.name && value.stat.baseStat === snorlaxData) {
      accumulator = value;
    }
    return arr;
  }, '');
